...
def should_continue(state: AgentState) -> str:
    ...
    if state.is_relevant == "yes":
        ...
        if state.iteration_count >= 1: # Allow only one re-try
...
def run_agent():
    ...
    with SqliteSaver.from_conn_string(CHECKPOINT_DB_PATH) as memory:
        ...
        workflow.add_node("add_initial_query_to_history", add_initial_query_to_history_node) # New node
        ...
        workflow.add_node("summarize_current_turn", summarize_current_turn_node) # New node
        ...
        workflow.add_edge("load_user_profile", "add_initial_query_to_history") # Connect new node
        workflow.add_edge("add_initial_query_to_history", "summarize_history") # Connect new node
        ...
        workflow.add_edge("grade_retrieval", "summarize_current_turn") # New edge
